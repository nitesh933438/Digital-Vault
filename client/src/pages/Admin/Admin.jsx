...
export default function Admin() {
  ...
  // Keep the admin drawer deterministic across phones, tablets and desktop
  // resizing. A drawer should never remain visually open after crossing the
  // responsive breakpoint, and opening it should not lock the page itself.
  useEffect(() => {
    ...
    window.addEventListener("orientationchange", closeOnWideViewport, { passive: true });
    window.addEventListener("keydown", onKeyDown);
    return () => {
      window.removeEventListener("resize", closeOnWideViewport);
      window.removeEventListener("orientationchange", closeOnWideViewport);
  ...
  useEffect(() => {
    ...
    const shouldLock = isDrawerViewport && mobileOpen;
      ...
      <main className="admin-main">
        <header className="admin-topbar">
          <div className="admin-topbar-left">
            <button className="admin-menu-btn" onClick={() => setMobileOpen(v => !v)} aria-label={mobileOpen ? "Close admin menu" : "Open admin menu"}>{mobileOpen ? <FaTimes /> : <FaBars />}</button>
